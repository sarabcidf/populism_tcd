...
# Calculate the total number of sentences in the dataset (assuming data_sentences is now your DataFrame)
N_sentences = len(data)

# Initialize a dictionary to count occurrences of each word
counts = defaultdict(int)

# Iterate through each sentence to count word frequencies
...
# Filter out infrequent and short words
to_del = [word for word in counts if counts[word] <= 4 or len(word) <= 2]
for word in to_del:
    del counts[word]

# Create a list of words that remain after filtering
...
# Determine the number of unique words left after filtering
N = len(word_index)

# Create matrices for storing the Bag of Words representation and labels
X = np.zeros((N_sentences, N))
Y = np.zeros(N_sentences)
# Initialize a list to store political orientations
orientations = []
i = 0

# Iterate through each sentence again for creating the Bag of Words matrix and assigning labels
for index, row in data.iterrows():
    clean_text = row['clean_text'].split()
    ...
    # Determine political orientation based on gps_Q3.1
...
p_train = 0.7
random_state = 1
n_jobs = 8 # Authors originally use 8 (I only got 8 total)
...
with open('best_random_forest_model_all.pkl', 'wb') as file:
