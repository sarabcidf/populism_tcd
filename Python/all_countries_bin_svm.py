...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, train_size=p_train, random_state=random_state)
...
best_score = 0
best_params = {}
pbar = tqdm(total=len(param_grid['C']) * len(param_grid['kernel']) * n_splits)
...
with open('best_svm_model_all_bin.pkl', 'wb') as file:
