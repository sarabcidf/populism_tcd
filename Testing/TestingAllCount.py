...
# Looking at avg predicted scores by party
party_avg_scores = test_data.groupby('mp_partyname')['predicted_score'].mean().reset_index()
party_avg_scores = party_avg_scores.sort_values(by='predicted_score', ascending=False)
party_avg_scores

# Scaling/adjusting predictions
...
# Defining the range and statistics of your target variable
target_min, target_max = 1.5, 8.6
target_mean = np.mean(Y_test)
target_std = np.std(Y_test)

# Adjusting the predictions
adjusted_predictions = adjust_predictions(predictions, target_mean, target_std, target_min, target_max)
mse_adjusted = mean_squared_error(Y_test, adjusted_predictions)

# Adding adjusted predictions to test data
test_data['predicted_score'] = adjusted_predictions

# Looking at avg adjusted predicted scores by party
party_avg_scores_adjusted = test_data.groupby('mp_partyname')['predicted_score'].mean().reset_index()
party_avg_scores_adjusted = party_avg_scores_adjusted.sort_values(by='predicted_score', ascending=False)
party_avg_scores_adjusted

# ## Exploring parameters and evaluating the model 
